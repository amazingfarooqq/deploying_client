...
// console.log(userInfo);
  ...
  return (
    ...
    <Link style={{position: 'absolute'}} className="navbar-brand p-3" to="/"> <img className="logo" width="100px" src="https://santastellarealestate.com/wp-content/uploads/2021/06/logo-transparent.png" alt="logo" /> </Link>
      ...
      <div className="row text-end">
        ...
        <div className="col-12 py-2 second_line_nav">
          ...
          <div className="navbar-nav">
            <Link className='nav-link px-4 text-dark fs-5' to="/">Home</Link>
            <Link className='nav-link px-4 text-dark fs-5' to="/buy">Buy</Link>

            {userInfo && userInfo.role === 'owner' ?
            <Link className='nav-link px-4 text-dark fs-5' to="/dashboard">Owner Dashboard</Link>   
            : userInfo && userInfo.role === 'admin' ? 
            <Link className='nav-link px-4 text-dark fs-5' to="/dashboard">Admin Dashboard</Link>   
            : userInfo && userInfo.role === 'user' ?
            <Link className='nav-link px-4 text-dark fs-5' to="/dashboard">Dashboard</Link> : ''
            }
            <Link className='nav-link px-4 text-dark fs-5' to="/sell">Sell</Link>
            <Link className='nav-link px-4 text-dark fs-5' to="/contact">Contact</Link>
            <Link className='nav-link px-4 text-dark fs-5' to="/about">About</Link>
            {userInfo &&
            <>
              {active ? <button className='btn btn-secondary' title="Disconnect From Wallet" onClick={disconnect}>{`ChainID: ${chainId} ${account.slice(0,5)}...${account.slice(-5)}`}</button> : 
              <ModalBootstrap 
